<template>
    ...
    <a-table :columns="columns" :data-source="dataSource" bordered>
        <template #bodyCell="{ column, text, record }">
            <template v-if="['core', 'pioneer', 'middleArmy','numberOfLimitUp','middleArmy','increase','genreTrends','persistence'].includes(column.dataIndex)">
                <div>
                    <a-input
          ...
          <span v-if="editableData[record.key]">
            <a-typography-link @click="save(record.key); console.log('Saving key:', record.key)">保存</a-typography-link>
...
const props = defineProps({
 ...
 id: Number
    ...
    const save = key => {
        console.log('id',props.id.toString())
        if (!props.id)
    {
